#!/bin/sh
# libscraps.sh - shared runtime for the ScrapLinux Package Manager
# POSIX sh only. Must run under busybox ash with no GNU utilities present.
# shellcheck shell=sh disable=SC2039

SCRAPS_VERSION="1.4.34"
SCRAPS_FORMAT="2"

: "${SCRAPS_ROOT:=/}"
# SCRAPS_CONF/SCRAPS_REPOD stay host-absolute on purpose even under --root: they
# are the *tool's* configuration (which repos exist, how many jobs, color),
# read from wherever scraps itself is actually running - during a target_scraps
# install that is still the live session, since the target's own /etc/scraps
# may not even have anything in it yet. SCRAPS_DB/SCRAPS_CACHE/SCRAPS_LOG describe
# the state of a specific root's own filesystem instead (what is installed
# there, what is cached there), so they follow SCRAPS_ROOT by default. Getting
# this wrong meant `SCRAPS_ROOT=$TARGET scraps ins ...` recorded every install
# into the *caller's* /var/lib/scraps instead of the target's - the target
# filesystem would boot with a completely empty package database despite
# every file actually being there.
: "${SCRAPS_CONF:=/etc/scraps/scraps.conf}"
: "${SCRAPS_REPOD:=/etc/scraps/repos.d}"
: "${SCRAPS_DB:=${SCRAPS_ROOT%/}/var/lib/scraps}"
: "${SCRAPS_CACHE:=${SCRAPS_ROOT%/}/var/cache/scraps}"
...
: "${SCRAPS_PORTS:=https://ports-scraplinux.apiwow.net/ALL}"
...
#
#   scraplinux  the default - apt's wording inside aeryn's frame
#   apt     Debian's shape
...
# ------------------------------------------------------------------- theming
#
# SCRAPS_STYLE names either one of the built-in styles or a theme file. A theme
# is a POSIX sh fragment in /etc/scraps/themes/<name>.theme (or
# ~/.config/scraps/themes/<name>.theme, which wins) that sets any of the T_*
...
# the terminal's business, not the package manager's - scraps writes text and
...
#
# A theme file is sourced, so it can also set SCRAPS_JOBS or anything else -
...
scraps_load_theme() {
	...
	scraplinux|apt|aeryn|pacman|void|solus) scraps_theme_defaults; return 0 ;;
	esac
	# Not a built-in: it names a theme file.
	case "$SCRAPS_STYLE" in
	""|.|..|*/*) SCRAPS_STYLE=scraplinux; scraps_theme_defaults; return 0 ;;
	esac
	for _t in "${XDG_CONFIG_HOME:-$HOME/.config}/scraps/themes/$SCRAPS_STYLE.theme" \
	          "/etc/scraps/themes/$SCRAPS_STYLE.theme"; do
		if [ -r "$_t" ]; then
			# shellcheck disable=SC1090
			. "$_t"
			SCRAPS_THEME_NAME=$SCRAPS_STYLE
			SCRAPS_STYLE=custom
			scraps_theme_defaults
			return 0
		fi
	done
	printf 'scraps: no theme named %s - using the default\n' "$SCRAPS_STYLE" >&2
	SCRAPS_STYLE=scraplinux
	scraps_theme_defaults
}
...
scraps_log() {
	[ -w "$(dirname "$SCRAPS_LOG")" ] 2>/dev/null || return 0
	printf '[%s] %s\n' "$(date '+%Y-%m-%d %H:%M:%S')" "$*" >>"$SCRAPS_LOG" 2>/dev/null || :
}
...
# Downloader abstraction. ScrapLinux ships busybox wget on the ISO and curl in base.
#
...
# only one a freshly installed ScrapLinux system has, and it implements exactly
...
dl_bar() {
	...
	_dlb_pid=$!
	_dlb_t0=$(date '+%s')
	# Most ScrapLinux packages are a few hundred KB and finish inside a second -
...
# style inside it. Nothing here changes what scraps does - the resolver, the
...
ui_summary() {
	...
	solus)
		...
		;;
	aeryn|scraplinux|*)
		# scraplinux is apt's wording inside aeryn's frame: the box says what is
...
ui_cached() {
	case "$SCRAPS_STYLE" in
	apt|scraplinux) printf 'Get:%s %s [cached]\n' "${3:-1}" "$2" ;;
...
ui_install() {
	case "$SCRAPS_STYLE" in
	apt|scraplinux) printf 'Setting up %s (%s)\n' "$2" "$3" ;;
...
ui_done() {
	...
	apt)
		...
		printf "Done. Undo with 'scraps rollback %s'.\n" "$3" ;;
	pacman)
		printf '\n'
		msg "Done. Undo with 'scraps rollback $3'" ;;
	void)
		printf '\n%s[*]%s Done - undo with: scraps rollback %s\n' \
			"$A_TEAL$C_B" "$C_R" "$3" ;;
	solus)
		printf '\nInstall completed. Undo with: scraps rollback %s\n' "$3" ;;
	custom)
		printf '\n  %s%s%s %s '"'"'%s'"'"' %s\n' "$A_MINT" "$T_MARK_OK" "$C_R" "Package" "$1" "$T_DONE"
		printf '  %sundo with: scraps rollback %s%s\n' "$A_GREY" "$3" "$C_R" ;;
	aeryn)
...
# FILES list get computed from what is actually there (see scraps's own
...
# ever runs against the real $SCRAPS_ROOT, after everything has merged.
untar() {
	archive=$1 dest=$2
	...
	# straight onto $SCRAPS_ROOT (see install_one). Checked against the listing
...
untar_src() {
	archive=$1 dest=$2
	# _HOST_MKDIR is scraps-build's captured pre-sysroot-PATH mkdir (see its
	# own comment on why); plain `scraps` never sets it, so this still means
...
# above is immune) replace ScrapLinux's merged-/usr symlink with a real directory
...
scraps_load_conf() {
	[ -f "$SCRAPS_CONF" ] || return 0
	# A caller's explicit environment override must never lose to a plain
	# assignment sitting in the config file - target_scraps's
	# SCRAPS_ROOT="$TARGET", or anything else scratch-rooting an install, has
	# to win over scraps.conf's own "SCRAPS_ROOT=/" line. Sourcing the file
	...
	for _v in SCRAPS_ROOT SCRAPS_DB SCRAPS_CACHE SCRAPS_BUILDROOT SCRAPS_REPOD; do
	...
	# SCRAPS_STYLE may have just been set (or changed) by the config, and a
...
# files with "pkgurl", and scraps fetches "<pkgurl>/<file>" instead of
# "<url>/<arch>/<file>". The index stays where it was, small and in the tree.
scraps_repo_pkgurl() {
	_pu_want=$1
	_pu=$(for f in "$SCRAPS_REPOD"/*.repo; do
		[ -f "$f" ] || continue
...
#
# ScrapLinux's own repositories ship as "required" - they are signed, so anything
...
# signature ScrapLinux's key would match.
scraps_repo_sigmode() {
	_sm_want=$1
	_sm=$(for f in "$SCRAPS_REPOD"/*.repo; do
		[ -f "$f" ] || continue
		n=$(meta "$f" name); [ -n "$n" ] || n=$(basename "$f" .repo)
		[ "$n" = "$_sm_want" ] || continue
		meta "$f" sig
...
# and the per-package checksum scraps verifies afterwards is taken from that
...
#
# The key lives in /etc/scraps/keys and arrives with scraplinux-base, so it is on
...
scraps_verify_index() {
	...
	_vi_mode=$(scraps_repo_sigmode "$_vi_name")
	[ "$_vi_mode" = off ] && return 0

	_vi_keys=""
	for k in "$SCRAPS_ROOT"/etc/scraps/keys/*.pub; do
		[ -f "$k" ] || continue
		_vi_keys="$_vi_keys $k"
	done
	if [ -z "$_vi_keys" ]; then
		[ "$_vi_mode" = required ] || return 0
		VERIFY_ERR="no public key in /etc/scraps/keys to check it against"
		return 1
	fi
	...
	rm -f "$_vi_file.sig"
	VERIFY_ERR="the signature does not match any key in /etc/scraps/keys"
	return 1
}
...
# A recipe is shell that scraps-build sources. A static host answers a missing
...
# index: telling someone to run 'scraps fetch all' when they synced ten minutes
# ago sends them to do it again for nothing, and the real answer is that the
# package does not exist.
idx_age_days() {
	_ia_new=""
	for _ia_f in "$SCRAPS_DB"/sync/*.idx; do
		[ -f "$_ia_f" ] || continue
...
not_found_hint() {
	...
	if [ "${_nf_age:-999}" -ge 1 ]; then
		printf " - the package lists are %s day(s) old, try 'scraps fetch all'" "$_nf_age"
	fi
}
...
# "mkdir $SCRAPS_DB/local/$p" and the end of the transaction left a directory
# with no DEPS and no PKGINFO behind, and this said "installed" for it
# forever after - `scraps ins vim` would report vim was already there while
# `scraps deps vim` died on the missing DEPS file, and the dependency tree
# marked it satisfied for everything that needed it.
is_installed() { [ -f "$SCRAPS_DB/local/$1/PKGINFO" ]; }
...
scraps_partials() {
	for _pt_d in "$SCRAPS_DB"/local/*/; do
		[ -d "$_pt_d" ] || continue
...
installed_version() {
	[ -f "$SCRAPS_DB/local/$1/PKGINFO" ] || return 1
	v=$(meta "$SCRAPS_DB/local/$1/PKGINFO" version)
	r=$(meta "$SCRAPS_DB/local/$1/PKGINFO" release)
	[ -n "$r" ] && printf '%s-%s\n' "$v" "$r" || printf '%s\n' "$v"
}

is_held() { [ -f "$SCRAPS_DB/hold/$1" ]; }
...
scraps_init_db() {
	...
	mkdir -p "$SCRAPS_CACHE/pkg" "$SCRAPS_CACHE/src" "$SCRAPS_CACHE/build"
	mkdir -p "$SCRAPS_ROOT/usr/lib/scraps/staged" 2>/dev/null || :
}
